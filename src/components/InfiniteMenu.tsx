// Copied in from React Bits Infinite Menu (github.com/DavidHDev/react-bits,
// src/ts-tailwind/Components/InfiniteMenu/InfiniteMenu.tsx @ main, 2026-07-07, MIT).
// Vitrine customizations: destroy() teardown (rAF cancel + GL context release),
// onSelect callback instead of window.open, overlay restyled to site tokens,
// items carry a slug for detail-view routing.
  ...
  return (
    <div className="relative w-full h-full">
      ...
      {activeItem && (
        <>
          <h2
            className={`select-none absolute left-6 md:left-12 top-1/2 -translate-y-1/2 max-w-[8ch] font-display text-4xl md:text-6xl leading-none tracking-tight text-bone transition-all ease-out ${overlayVisibility}`}
          >
            {activeItem.title}
          </h2>

          <p
            className={`select-none absolute right-6 md:right-12 top-1/2 -translate-y-1/2 max-w-[24ch] text-sm text-bone/60 text-right transition-all ease-out ${overlayVisibility}`}
          >
          ...
          <button
            ...
            className={`absolute left-1/2 -translate-x-1/2 bottom-14 z-10 grid h-14 w-14 place-items-center rounded-full border border-bone/40 bg-ink/60 text-bone backdrop-blur-sm transition-all ease-out hover:bg-bone hover:text-ink ${
              isMoving ? 'opacity-0 pointer-events-none scale-75 duration-100' : 'opacity-100 scale-100 duration-500'
            }`}
          >
